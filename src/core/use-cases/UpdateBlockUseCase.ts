...
export class UpdateBlockUseCase {
  ...
  async execute(blockId: string, updates: IUpdateBlockDto): Promise<IBlockDto | null> {
  ...
  private validateSettings(settings: Partial<Record<string, unknown>>): void {
  ...
  private validateProps(props: Partial<Record<string, unknown>>): void {
  ...
  private validateStyle(style: Partial<Record<string, string | number>>): void {
    Object.entries(style).forEach(([key, value]) => {
      if (typeof value !== 'string' && typeof value !== 'number') {
        throw new TypeError(`Invalid style value for key '${key}': must be string or number`);
      }
    });
  }
}
