...
## Сравнение с Core API (`api-usage`)

| Функция | Core API | Vue3 + Vite |
|---------|----------|-------------|
| Готовый UI пакета | Нет (свой UI) | Да |
| Vue компоненты | Свои SFC | SFC блоков + BlockBuilder |
| Сборка | Vite | Vite |
| HMR | Да | Да |
...
- Блок **«Текстовый блок (простой)»** содержит поле **«Темы (select, множественный выбор)»** — проверка `type: 'select'` + `multiple: true`
